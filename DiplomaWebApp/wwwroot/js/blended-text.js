...
var createLetterLayers = function createLetterLayers(array) {
  return array.map(function (letter) {
    ...
    for (var i = 1; i <= 2; i++) {
      ...
      if (letter == ' ') {
        layer += '<span class="spanclass space"></span>';
      } else {
        layer += '<span class="spanclass letter-' + i + '">' + letter + '</span>';
      }
    }
    return layer;
  });
...
var outputLayers = new Promise(function (resolve, reject) {
  document.getElementById('blended-text').innerHTML = createLetterContainers(createLetterLayers(createLetterArray(text))).join('');
  resolve();
});

// then adjust width and height of each letter
var spans = Array.prototype.slice.call(document.getElementsByClassName('spanclass'));
